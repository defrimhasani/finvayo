...
import { api } from "../api";
import { categoryActivity, type CategoryActivity, type CashEntry, type Direction, type Financials, type Overview, PageHeader, previewEntries, previewOverview, UpcomingLedger, fail, today } from "../appData";
...
export default function AppPage() {
  ...
  return <AppShell activePage="overview" preview={preview}><main className="app-main" id="app-main">
    ...
    <section className="category-activity-section" aria-labelledby="category-activity-title">
      <div className="transactions-heading"><div><p className="app-kicker">Activity mix</p><h2 id="category-activity-title">Activity by category</h2></div><p>Recorded and planned entries currently in your workspace, grouped by category.</p></div>
      <div className="category-activity-grid">
        <CategoryActivityChart title="Money in" direction="inflow" activity={inflowActivity} currency={currency} />
        <CategoryActivityChart title="Money out" direction="outflow" activity={outflowActivity} currency={currency} />
      </div>
    </section>
...
function CategoryActivityChart({ title, direction, activity, currency }: { title: string; direction: Direction; activity: CategoryActivity[]; currency: string }) {
  const maximum = Math.max(...activity.map((item) => item.amountMinor), 1);
  const total = activity.reduce((sum, item) => sum + item.amountMinor, 0);
  return <article className={`category-chart ${direction}`}>
    <div className="category-chart-heading"><div><span>{title}</span><strong>{money(total, currency)}</strong></div><small>{activity.reduce((sum, item) => sum + item.count, 0)} entries</small></div>
    {activity.length === 0 ? <p className="transaction-empty">No {direction === "inflow" ? "income" : "expense"} activity has been categorized yet.</p> : <div className="category-bars" role="img" aria-label={`${title} by category. ${activity.map((item) => `${item.label}: ${money(item.amountMinor, currency)}`).join("; ")}`}>
      {activity.slice(0, 6).map((item) => <div className="category-bar-row" key={item.category}>
        <div className="category-bar-label"><span>{item.label}</span><strong>{money(item.amountMinor, currency)}</strong></div>
        <div className="category-bar-track" aria-hidden="true"><span style={{ width: `${Math.max(4, item.amountMinor / maximum * 100)}%` }} /></div>
        <small>{item.count} {item.count === 1 ? "entry" : "entries"}</small>
      </div>)}
    </div>}
  </article>;
}
